...
CString CapitalizeBaseclass(const CString& baseclass);
...
/*
  ...
  case RaceNotInParty:
    {
      event.NameAndQuotedNonBlank("trigger_data",     CapitalizeRace(raceID));
    };
    break;
  case ClassInParty:
  case ClassNotInParty:
    {
      event.NameAndQuotedNonBlank("trigger_data",    CapitalizeClass(classBaseclassID.classID));
    };
    break;
  case BaseclassInParty:
  case BaseclassNotInParty:
    {
      event.NameAndQuotedNonBlank("trigger_data",    CapitalizeBaseclass(classBaseclassID.baseclassID));
    };
    break;
  case PartyHaveItem:
  case PartyNotHaveItem:
    {
      event.NameAndQuotedNonBlank("trigger_data",    CapitalizeItem(itemID));
    }
    break;
  case NPCInParty:
  case NPCNotInParty:
    {
      event.NameAndQuotedNonBlank("trigger_data",    CapitalizeCharacterID(characterID));
    };
  case SpellMemorized:
    {
      event.NameAndQuotedNonBlank("trigger_data",        CapitalizeSpell(memorizedSpellID));
    };
  case GenderInParty:
...
void EVENT_CONTROL::Export(JWriter& jw)
{
  ...
  {
    jw.NameAndQuotedNonBlank(JKEY_RACE,         (LPCSTR)CapitalizeRace(raceID));
  };
  jw.NameAndQuotedNonBlank(JKEY_CLASS,        (LPCSTR)CapitalizeClass(classBaseclassID.classID));
  jw.NameAndQuotedNonBlank(JKEY_BASECLASS,    (LPCSTR)CapitalizeBaseclass(classBaseclassID.baseclassID));
  jw.NameAndQuotedNonBlank(JKEY_ITEM,         (LPCSTR)CapitalizeItem(itemID));
  jw.NameAndQuotedNonBlank(JKEY_CHARACTER,    (LPCSTR)CapitalizeCharacterID(characterID));
  jw.NameAndQuotedNonBlank(JKEY_SPELL,        (LPCSTR)CapitalizeSpell(memorizedSpellID));
...
void PIC_DATA::operator =(const CString& str)
{
  ...
  filename = str.Left(j);
#pragma warning (suppress:4996)
  sscanf(((LPCSTR)str)+j+1, "%d,%d,%d,%d",  &dly, &n, &w, &h);
  picType = SmallPicDib;
  timeDelay = dly;
  NumFrames = n;
  FrameWidth = w;
    ...
    for (i=0; i<artMap.GetSize(); i++)
    {
      if (artMap[i].strings[0] == filename)
      {
        ...
        else
        {
          ...
          if (artMap[i].strings.GetSize() >= 3)
          {
            CString filetype;
            filetype = artMap[i].strings[2];
            if (filetype.CompareNoCase("Sprite") == 0)
            {
              picType = SpriteDib;
            }
            if (artMap[i].strings.GetSize() >= 4)
            {
              CString picsize;
              picsize = artMap[i].strings[3];
#pragma warning (suppress:4996)
              sscanf((LPCSTR)picsize, "%d,%d", &FrameWidth, &FrameHeight);
            };
          };
        };
        break;
...
void ITEM::Export(JWriter& jw)
{
  ...
  jw.NameAndValue (JKEY_ITEMID     ,CapitalizeItem(itemID));
...
void CHARACTER_SPELL::Export(JWriter& jw)
{
  jw.StartList();
  jw.Linefeed(false);
  jw.NameAndValue(JKEY_NAME,CapitalizeSpell(spellID));
...
void ITEM_LIST::InsertItem(const CString& itemName)
{
  ITEM item;

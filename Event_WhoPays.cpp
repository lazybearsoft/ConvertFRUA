...
void WHO_PAYS_EVENT_DATA::Read(JReader& event, JReader& details)
{
  ...
  details.NameAndValue(JKEY_SUCCESSEVENT, successChain);
  {
    ...
    if (temp == "backup one step") successAction = BackupOneStep;
    if (temp == "none") successAction = NoAction;
  };
  ...
  details.NameAndValue(JKEY_FAILEVENT, failChain);
  {
    ...
    if (temp == "backup one step") failAction = BackupOneStep;
    if (temp == "none") failAction = NoAction;
  };

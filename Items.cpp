...
CString CapitalizeSpell(const CString& spellName);
...
void ITEM_DATA::Clear(BOOL ctor)
{
  ...
  Recharge_Rate.Format("%d", irrNever);
  IsNonLethal=FALSE;
...
CString UniqueItemName(const CString& name)
{
  LINE_DATA *pData;
  LINE_ENTRY *pLine;
  pData = defaultItemData.Find("name", name);
  if (pData != NULL) return name;
  pData = jsonItemData.Find("name", name);
  if (pData != NULL) return name;
  pData = defaultItemData.Find("id name", name);
  if (pData != NULL)
  {
    pLine = (*pData)["name"];
    if (pLine != NULL) return pLine->value;
  };
  pData = jsonItemData.Find("id name", name);
  if (pData != NULL)
  {
    pLine = (*pData)["name"];
    if (pLine != NULL) return pLine->value;
...
DEFID ( "ROF per round"           ,float       ,(int)ROF_Per_Round);
...
#pragma warning (suppress:4996)
  f = fopen(binPath + "FRUA_Conversion_Items.txt", "r");
  if (f == NULL)
  {
    char line[200];
    printf("Cannot open \"FRUA_Conversion_Items.txt\"\n");
    printf("Press Enter ---->");
    fgets(line, 190,stdin);
    printf("\n");
    return;
  };
...
void RenameJsonItems(void)
{
  ...
  for (i=0; i<n; i++)
  {
    ...
    if (pJsonIdNameLine->value == "Long Sword +2")
    {
      int kkk=1;
    };
  ...
  
  {
    ...
    for (i=0; i<n; i++)
    {
      ...
      for (j = 0; j < m; j++)
      {
        ...
        if (pDefaultLine->key == "name")
        {
          fprintf(f, "%s = %s\n", (LPCSTR)pDefaultLine->key, (LPCSTR)CapitalizeItem(pDefaultLine->value));
        }
        else if (pDefaultLine->key == "id name")
        {
          fprintf(f, "%s = %s\n", (LPCSTR)pDefaultLine->key, (LPCSTR)CapitalizeItem(pDefaultLine->value));
        }
        else if (pDefaultLine->key == "baseclass")
        {
          fprintf(f, "%s = %s\n", (LPCSTR)pDefaultLine->key, (LPCSTR)CapitalizeBaseclass(pDefaultLine->value));
        }
        else if (pDefaultLine->key == "spell effect")
        {
          fprintf(f, "%s = %s\n", (LPCSTR)pDefaultLine->key, (LPCSTR)CapitalizeSpell(pDefaultLine->value));
        }
        else
        {
          fprintf(f, "%s = %s\n", (LPCSTR)pDefaultLine->key, (LPCSTR)pDefaultLine->value);
        };
      };
      fprintf(f,"\\(END)\n");
    };
    ...
    for (i=0; i<n; i++)
    {
      ...
      fprintf(f,"\n\\(BEGIN)\n");
      {
        ...
        for (j=0; j<m; j++)
        {
          ...
          if (pJsonNameLine->key == "name")
          {
            fprintf(f, "%s = %s\n", (LPCSTR)pJsonNameLine->key, (LPCSTR)CapitalizeItem(pJsonNameLine->value));
          }
          else if (pJsonNameLine->key == "id name")
          {
            fprintf(f, "%s = %s\n", (LPCSTR)pJsonNameLine->key, (LPCSTR)CapitalizeItem(pJsonNameLine->value));
          }
          else if (pJsonNameLine->key == "baseclass")
          {
            fprintf(f, "%s = %s\n", (LPCSTR)pJsonNameLine->key, (LPCSTR)CapitalizeBaseclass(pJsonNameLine->value));
          }
          else
          {
            fprintf(f, "%s = %s\n", (LPCSTR)pJsonNameLine->key, (LPCSTR)pJsonNameLine->value);
          };
        };
      };
      fprintf(f,"\\(END)\n");

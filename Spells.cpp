...
void ConvertOneSpell(JReader& spell)
{
  ...
  Line("Key"              ,Int("id"));
  {
    ...
    Line("name", CapitalizeSpell(name) + "|" + CapitalizeBaseclass(spellBook) + "|" + level + "|" + Str("id"));
  };
...
*/
  ...
  WriteLine(f, "target linger art"   , "");
  {
    ...
    Line("school"                        , CapitalizeSchool(school));
    ...
    WriteLine(f, "cast by"             , CapitalizeBaseclass(castBy));
  };
...
#pragma warning (suppress:4996)
  f = fopen(binPath + "FRUA_Conversion_Spells.txt", "r");
  if (f == NULL)
  {
    char line[200];
    printf("Cannot open \"FRUA_Conversion_Spells.txt\"\n");
    printf("Press Enter ---->");
  ...
  ReadDCDefaultSpells(f);
  {
    ...
    for (i=0; i<n; i++)
    {
      fprintf(f,"\n\\(BEGIN)\n");
      {
        ...
        for (j=0; j<m; j++)
        {
          if (defaultSpellData[i].key[j] == "name")
          {
            fprintf(f, "%s = %s\n", (LPCSTR)defaultSpellData[i].key[j], (LPCSTR)CapitalizeSpell(defaultSpellData[i].value[j]));
          }
          else if (defaultSpellData[i].key[j] == "school")
          {
            fprintf(f, "%s = %s\n", (LPCSTR)defaultSpellData[i].key[j], (LPCSTR)CapitalizeSchool(defaultSpellData[i].value[j]));
          }
          else if (defaultSpellData[i].key[j] == "cast by")
          {
            fprintf(f, "%s = %s\n", (LPCSTR)defaultSpellData[i].key[j], (LPCSTR)CapitalizeBaseclass(defaultSpellData[i].value[j]));
          }
          else
          {
            fprintf(f, "%s = %s\n", (LPCSTR)defaultSpellData[i].key[j], (LPCSTR)defaultSpellData[i].value[j]);
          };
        };
      };
      fprintf(f,"\\(END)\n");

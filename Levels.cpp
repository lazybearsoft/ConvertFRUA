...
void ConvertOneLevel(JReader& global, JReader& level, JWriter& jw)
{
  ...
  };
  {
    ...
    while (zones.Optional(), zones.NextEntry())
    {
      ...
      jw.NameAndValue("zoneName"              , CapitalizeZoneName(zoneName));
    ...
    };
    {
      ...
      for (i=9; i<=16; i++)
      {
        ...
        jw.StartList();
          ...
          jw.NameAndValue("zoneName", CapitalizeZoneName(name));
...
void ConvertLevels(JReader& global, JReader& jr, CFile *scriptFile)
{
  int levelNum;
  CFile output;
  CString fname;
  while (jr.Optional(), jr.NextEntry())
  {
    ...
    jw.StartList();
    {
      ...
      jw.NameAndValue("ConvertFRUA Version", ConvertFRUAVersion);
    };
    ConvertOneLevel(global, jr, jw);
    {
      char line[100];
#pragma warning (suppress:4996)
      sprintf(line, "Level/Level List/%d/Import/Level%03d.txt\n", levelNum, levelNum);
      scriptFile->Write(line, strlen(line));
    };
    jw.EndList();
    output.Close();
